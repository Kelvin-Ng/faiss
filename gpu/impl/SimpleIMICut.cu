...
#define RUN_L2(TYPE_T, TYPE_TVEC, INPUT)                                            \
  ...
  if ( (squareLen % 4 == 1) && (totalLen % 4 == 3) ){
    ...
    if(numThreads % 32 != 0) numThreads += (32 - numThreads % 32);
...
void runSimpleIMICut(const Tensor<float, 3, true>& input,
                     ...
                     int totalLen,
                     cudaStream_t stream) {
  auto  input2 = input.narrow(2, 0, totalLen);
  if (input.canUseIndexType<int>()) {
    runSimpleIMICut<float, int, int>(input2, output, squareLen, totalLen, stream);
  } else {
...
void runSimpleIMICut(const Tensor<half, 3, true>& input,
                     Tensor<int, 2, true>& output,
                     int squareLen,
                     int totalLen,
                     cudaStream_t stream) {
