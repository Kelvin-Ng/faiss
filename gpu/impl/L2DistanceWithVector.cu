...
#include "L2DistanceWithVector.cuh"
...
__global__ void l2DistanceWithVector(Tensor<TVec, 2, true, int64_t> input,
                                     Tensor<TVec, 1, true, int64_t> vec,
    ...
    for (int64_t row = 0; row < input.getSize(0) - rowStart; ++row) {
      if (NormLoop) {
        ...
        for (int64_t col = threadIdx.x;
          ...
          TVec val = Math<TVec>::sub(input[rowStart + row][col], vec[col]);
          val = Math<TVec>::mul(val, val);
          rowNorm[0] = Math<T>::add(rowNorm[0], Math<TVec>::reduceAdd(val));
        }
      } else {
        TVec val = Math<TVec>::sub(input[rowStart + row][threadIdx.x], vec[threadIdx.x]);
        val = Math<TVec>::mul(val, val);
...
#pragma unroll
        for (int row = 0; row < RowTileSize; ++row) {
          tmp[row] = Math<TVec>::sub(input[rowStart + row][col] - vec[col]);
        }
...
#pragma unroll
      for (int row = 0; row < RowTileSize; ++row) {
        tmp[row] = Math<TVec>::sub(input[rowStart + row][threadIdx.x], vec[threadIdx.x]);
      }
...
template <typename T, typename TVec, typename int64_t>
void runL2DistanceWithVector(Tensor<T, 2, true, int64_t>& input,
                             Tensor<T, 1, true, int64_t>& vec,
                             Tensor<T, 1, true, int64_t>& output,
                             bool normSquared,
                             cudaStream_t stream) {
...
#define RUN_L2(TYPE_T, TYPE_TVEC, INPUT, VEC)                                \
  do {                                                                  \
    if (normLoop) {                                                     \
      if (normSquared) {                                                \
        l2DistanceWithVector<TYPE_T, TYPE_TVEC, int64_t, rowTileSize, true, true>      \
          <<<grid, block, smem, stream>>>(INPUT, VEC, output);               \
      } else {                                                          \
        l2DistanceWithVector<TYPE_T, TYPE_TVEC, int64_t, rowTileSize, true, false>     \
          <<<grid, block, smem, stream>>>(INPUT, VEC, output);               \
      }                                                                 \
    } else {                                                            \
      if (normSquared) {                                                \
        l2DistanceWithVector<TYPE_T, TYPE_TVEC, int64_t, rowTileSize, false, true>     \
          <<<grid, block, smem, stream>>>(INPUT, VEC, output);               \
      } else {                                                          \
        l2DistanceWithVector<TYPE_T, TYPE_TVEC, int64_t, rowTileSize, false, false>    \
          <<<grid, block, smem, stream>>>(INPUT, VEC, output);               \
      }                                                                 \
    }                                                                   \
  } while (0)
...
void runL2DistanceWithVector(Tensor<float, 2, true>& input,
                             Tensor<float, 1, true>& vec,
                             Tensor<float, 1, true>& output,
                             bool normSquared,
                             cudaStream_t stream) {
  if (input.canUseIndexType<int>()) {
    runL2DistanceWithVector<float, float4, int>(input, vec, output, normSquared, stream);
  } else {
    auto inputCast = input.castIndexType<long>();
    auto vecCast = vec.castIndexType<long>();
    auto outputCast = output.castIndexType<long>();
    runL2DistanceWithVector<float, float4, long>(inputCast, vecCast, outputCast, normSquared, stream);
  }
}

#ifdef FAISS_USE_FLOAT16
void runL2DistanceWithVector(Tensor<half, 2, true>& input,
                             Tensor<half, 1, true>& vec,
                             Tensor<half, 1, true>& output,
                             bool normSquared,
                             cudaStream_t stream) {
  if (input.canUseIndexType<int>()) {
    runL2DistanceWithVector<half, half2, int>(input, vec, output, normSquared, stream);
  } else {
    auto inputCast = input.castIndexType<long>();
    auto vecCast = vec.castIndexType<long>();
    auto outputCast = output.castIndexType<long>();
    runL2DistanceWithVector<half, half2, long>(inputCast, vecCast, outputCast, normSquared, stream);
  }
}
#endif
